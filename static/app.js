...
async function showScreen(id) {
    ...
    if (id !== 'player') {
        ...
        audio.src = '';
        isPlaying = false;
        document.getElementById('btn-play').innerHTML = '<img src="/static/svg/play.svg" class="button-icon-svg" alt="" aria-hidden="true" />';
    }
    window.scrollTo(0, 0);
    ...
    audio.play = function () {
        if (window._juceMode) {
            ...
            const p = enqueue(async (gen) => {
                ...
                isPlaying = true;
                document.getElementById('btn-play').innerHTML = '<img src="/static/svg/pause.svg" class="button-icon-svg" alt="" aria-hidden="true" />';
                const sm = window.slopsmith;
                if (sm) {
                    sm.isPlaying = true;
